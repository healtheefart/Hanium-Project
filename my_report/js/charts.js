function index_to_shortname( index ){
    return ["x_frame_options","allowed_methods"][index];
}

function index_to_severity( index ){
    return {"x_frame_options":"low","allowed_methods":"informational"}[index_to_shortname(index)];
}
...
function renderCharts() {
    ...
    c3.generate({
        ...
        axis: {
            x: {
                type: 'category',
                categories: ["Missing 'X-Frame-Options' header","Allowed HTTP methods"],
                tick: {
                    rotate: 15
                }
            },
